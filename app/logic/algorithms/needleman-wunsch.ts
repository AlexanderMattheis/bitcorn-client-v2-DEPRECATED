...
export default class NeedlemanWunsch extends LinearAlignment {
     ...
     * @override
     */
    protected computeMatrixAndScore(): void {
    ...
    private recursionFunction(aChar: string, bChar: string, i: number, j: number): number {
        let matchOrMismatch: number = aChar === bChar ? this.inputData.match : this.inputData.mismatch;

        let diagonalValue: number = this.outputData.matrix[i - 1][j - 1] + matchOrMismatch;
        let upValue: number = this.outputData.matrix[i - 1][j] + this.inputData.gap;
        let leftValue: number = this.outputData.matrix[i][j - 1] + this.inputData.gap;
